...
# start MIPS program
SOURCES = ["src/check.asm", "src/main.asm", "src/buffer.asm", "src/merge.asm", "src/move_check.asm", "src/move_left.asm", "src/complete_move.asm",
...
while True:
    ...
    if not proc.poll() is None or not gui.poll()is None:
        print("Supprocess died")
        sys.exit(1)

    # read the board
    board = []
    size = output.readline().replace('\n', '')
    if size != "16":
        print("Wrong formatted board")
        sys.exit(1)
